...
After cloning: `corepack enable` (once per machine), then `yarn install`. Default **`yarn preview`** is **production-shaped** (real R2 incremental cache, DO tag cache, **`REPRO_NESTED_STREAM_SERIAL_COLUMNS=0`**, Link **prefetch** on) so you can reproduce Wrangler issues quickly. Use **`yarn preview:without-inc-cache`** when you need a **stable** local loop — it uses **`REPRO_RESPONSE_KB=8`** per column so `/nested-stream` stays small enough for Wrangler Flight (see below). **`.dev.vars`** supplies **`OPENNEXT_DISABLE_REGIONAL_CACHE`**; **do not** put **`REPRO_RESPONSE_KB`** there unless you want one size for *all* previews — it **overrides** `cross-env` in scripts (OpenNext merges Wrangler / `.dev.vars` on top of `process.env`).
...
| Variable | Meaning |
|----------|---------|
| `REPRO_RESPONSE_KB` | Approximate payload size in **kilobytes** (default **512** if unset on Node **`yarn start` / `dev`**). Hard cap **16384** (16 MiB). **`yarn preview`** forces **`64`** (stress). **`yarn preview:without-inc-cache`** forces **`8`** per cached column so `/nested-stream` usually stays under Wrangler Flight limits. If you set this in **`.dev.vars`**, it **overrides** those scripts — a common reason “stable” preview still showed raw Lorem + “This page couldn’t load”. |
...
| `yarn preview:without-inc-cache` | **Stable local preview:** **`OPENNEXT_PREVIEW_DISABLE_INC_CACHE=1`**, **dummy tag cache + direct queue**, **`REPRO_NESTED_STREAM_SERIAL_COLUMNS=1`**, **`NEXT_PUBLIC_REPRO_NAV_PREFETCH=0`**, **`REPRO_RESPONSE_KB=8`** (keeps `/nested-stream` Flight small on Wrangler). |
